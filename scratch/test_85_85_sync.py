...
from scratch.test_optimal_tiered_detector import generate_network_benchmark_v5
...
def run_tier1_and_tier2_rules_85(featured, thresholds, spatial_z_dict):
    ...
    for station_id, g in df.groupby("station_id", sort=False):
        ...
        val_history = {p: deque(maxlen=5) for _, p in prefixes}
        faillow_streaks = {p: 0 for _, p in prefixes}
        last_spike_step = {p: -10 for _, p in prefixes}
        chatter_streaks = {p: 0 for _, p in prefixes}
        ...
        for i in range(m):
            ...
            # Tier 2: Psychrometric Clausius-Clapeyron Violation
            mv_confirmed = (z_T >= 2.2 and z_RH >= 2.2 and (z_T * z_RH >= 14.0) and abs(z_P) < 2.5)
            ...
            for col, prefix in prefixes:
                ...
                # Frozen: 5-step stagnation range (strict physical ADC limit)
                val_history[prefix].append(val)
                eps_freeze = 0.03 if prefix in ("temp", "pressure") else 0.06
                if len(val_history[prefix]) >= 5 and not dropout:
                    h_arr = np.array(val_history[prefix])
                    frozen = (np.max(h_arr) - np.min(h_arr) <= eps_freeze)
                else:
                    frozen = False

                # Spike: High-SNR Impulse Jump + Spatial outlier + Strict Recovery Suppression
                ...
                spike_thresh = PHYSICAL_SPIKE_LIMITS_85[col]
                z_cur = abs(z_T) if prefix == "temp" else (abs(z_P) if prefix == "pressure" else abs(z_RH))

                if step_diff >= spike_thresh and z_cur >= 4.0:
                    if i - last_spike_step[prefix] > 1:
                        spike = True
                        last_spike_step[prefix] = i
                    else:
                        spike = False
                        last_spike_step[prefix] = i
                else:
                ...
                # Unstructured chatter: persistent rapid volatility
                is_chatter_step = (step_diff >= 4.5 and z_cur >= 3.5)
                if is_chatter_step:
                    chatter_streaks[prefix] += 1
                else:
                    chatter_streaks[prefix] = 0
                chatter_confirmed = (chatter_streaks[prefix] >= 2) or (is_chatter_step and z_cur >= 5.5)
...
def test_85_85():
    ...
    detector_T = ConditionalMultivariateDriftDetector(glrt_thresh=44.0, cusum_thresh=34.0, allowance=0.50, persist_req=3)
    detector_RH = ConditionalMultivariateDriftDetector(glrt_thresh=54.0, cusum_thresh=40.0, allowance=0.60, persist_req=3)
    detector_P = ConditionalMultivariateDriftDetector(glrt_thresh=46.0, cusum_thresh=36.0, allowance=0.50, persist_req=3)
    artifact = joblib.load(ARTIFACTS_PATH)

    canonical_seeds = [42, 101, 202, 2024, 8888, 20260924, 45456231412727229999]
    results = []

    print(f"--- RUNNING 85/85 TARGET BENCHMARK ACROSS 7 CANONICAL SEEDS ---")
    ...
    for seed in canonical_seeds:
        ...
        for sid in STATION_TO_CLUSTER:
            ...
            # Gating drift flags with spatial standardized residual (requires |z| >= 2.6 to confirm drift)
            drift_T = res_T['online_predictions'] & (abs(Z[:, 0]) >= 2.6)
            drift_RH = res_RH['online_predictions'] & (abs(Z[:, 1]) >= 2.6)
            drift_P = res_P['online_predictions'] & (abs(Z[:, 2]) >= 2.6)
        ...
        featured_base, row_hard, row_rule_conf, row_fault_type = run_tier1_and_tier2_rules_85(featured.copy(), thresholds, spatial_z_dict)
    ...
    print("\n" + "=" * 80)
    print("7-SEED 85/85 BENCHMARK RESULTS")
    print("=" * 80)
