...
class RefinedConditionalDriftDetector:
    def __init__(
        self,
    ...
    def process_station_stream(self, u_T: np.ndarray, sig_cond_T: np.ndarray, timestamps: np.ndarray) -> dict:
        ...
        for i in range(n):
            ...
            if state == 0:
                ...
                # Clean exit when triggering ceases
...
def run_spatially_corroborated_rules(featured, thresholds, spatial_z_dict):
    ...
    for station_id, g in df.groupby("station_id", sort=False):
        ...
        # Spatial Z values for this station
        stn_spatial_z = spatial_z_dict.get(station_id, {})
        ...
        for i in range(m):
            ...
            # Multivariate Fix: Clausius-Clapeyron consistent level gating + peer deviation
            mv_level = (
            ...
            for col, prefix in prefixes:
                ...
                # Spike: Physical step jump + Spatial Corroboration (z_spatial >= 2.0)
                ...
                # Spatial check: True spikes deviate sharply from peers (z_spatial >= 2.0)
                # Diurnal solar jumps affect all peers equally (z_spatial ~ 0.9)
                z_sp = stn_spatial_z.get(col, np.zeros(m))[i] if col in stn_spatial_z else 0.0
                if conf > 0 and abs(z_sp) < 2.0:
                    conf = 0.0 # Suppress diurnal weather step jump
...
def evaluate_candidate(seed=42):
    print(f"=== TESTING REFINED CAUSAL DETECTOR ON SEED {seed} ===")
    ...
    detector = RefinedConditionalDriftDetector(glrt_thresh=30.0, cusum_thresh=24.0, allowance=0.45, persist_req=2)
    artifact = joblib.load(ARTIFACTS_PATH)

    data = generate_network_benchmark(regime='observable_v1', seed=seed, save_to_disk=False)
    
    # Compute spatial Z per station
    ...
    featured_base, row_hard, row_rule_conf, row_fault_type = run_spatially_corroborated_rules(featured.copy(), thresholds, spatial_z_dict)
