...
def run_tier1_and_tier2_rules_filtered(featured, thresholds, spatial_z_dict):
    ...
    for station_id, g in df.groupby("station_id", sort=False):
        ...
        for i in range(m):
            ...
            # Specialized Tier 2: Psychrometric Coupled Residual
            mv_coupled = (z_T >= 1.8 and z_RH >= 1.8 and (z_T * z_RH >= 4.0) and abs(z_P) < 2.2)
            mv_level = (
            ...
            mv_confirmed = (mv_streak >= 2) or (mv_coupled and (z_T * z_RH >= 6.0))
            ...
            for col, prefix in prefixes:
                ...
                # Spike: Physical transducer step difference + Spatial Outlier Check
                # True spike: step jump >= threshold AND spatial deviation |z| >= 2.0
                # Recovery point: step jump >= threshold BUT spatial deviation |z| < 1.5 (returned to ambient weather!)
                ...
                z_channel = z_T if prefix == "temp" else (z_P if prefix == "pressure" else z_RH)
                spike = (step_diff >= spike_thresh) and (abs(z_channel) >= 2.0)
...
def test_filtered_system(seed=42):
    ...
    featured_base, row_hard, row_rule_conf, row_fault_type = run_tier1_and_tier2_rules_filtered(featured.copy(), thresholds, spatial_z_dict)
    ...
    print("\n--- RESULTS ON SEED 42 WITH SPIKE RECOVERY FILTERING ---")
