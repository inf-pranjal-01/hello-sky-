...
class ConditionalMultivariateDriftDetector:
    """
    Answers: 'After explaining observed motion using other channels,
    is there statistically significant persistent drift remaining?'
    Combines:
      - Leave-one-channel-out conditional innovation u_c
      - Intercept-aware weighted Ramp-GLRT (Delta RSS >= 0, Lambda >= 0)
      - Causal directional CUSUM accumulators (C+, C-)
      - Trajectory sign coherence
      - Causal state machine (Normal, Suspect, Confirmed, Recovery)
    """
    def __init__(self, glrt_thresh=24.0, cusum_thresh=18.0, min_coherence=0.65, decay=0.88, allowance=0.40):
    ...
    def process_station_stream(self, u_c_arr, cond_std_arr, timestamps):
        ...
        for i in range(n):
            ...
            # 2. Intercept-Aware Weighted Ramp-GLRT (Invariants 7, 8, 9)
            max_lam = 0.0
            best_b = 0.0
            best_onset = i
            ...
            cusum_trigger = (active_cum >= self.cusum_thresh and is_coherent and abs(z_u[i]) >= 1.5)

            if state == 0:
                if glrt_trigger or cusum_trigger:
                    state = 2
                    recovery_streak = 0
                elif (max_lam >= 10.0 or active_cum >= 8.0) and abs(best_b) >= 0.02:
                    state = 1
                    recovery_streak = 0
            elif state == 1:
                if glrt_trigger or cusum_trigger:
                    state = 2
                    recovery_streak = 0
                elif max_lam < 5.0 and active_cum < 4.0:
                    state = 0
            elif state == 2:
                if abs(z_u[i]) < 1.0 and max_lam < 5.0 and active_cum < 4.0:
                    state = 3
                    recovery_streak = 1
            elif state == 3:
                recovery_streak += 1
                if recovery_streak >= 3 and abs(z_u[i]) < 1.0 and max_lam < 3.0:
                    state = 0
                    c_plus = 0.0
                    c_minus = 0.0
                    recovery_streak = 0
...
def execute_benchmark():
    ...
    # Thresholds calibrated from 99% clean empirical quantile
    theta_glrt = 24.0
    theta_cusum = 18.0
    detector = ConditionalMultivariateDriftDetector(glrt_thresh=theta_glrt, cusum_thresh=theta_cusum)
    ...
    print("Beginning 7-Seed Evaluation across Benchmark B...")
    ...
    for seed in canonical_seeds:
        ...
        # Stream Conditional Multivariate Drift Detector
        det_flags_list = []
        for sid in STATION_TO_CLUSTER:
            df = data[sid].sort_values('timestamp').reset_index(drop=True)
            cid = STATION_TO_CLUSTER[sid]
            ...
            # Standardized residuals for all 3 channels
            Z_list = []
            for p in joint_model.channels:
                y_hat, _ = normal_model.predict_target_robust(sid, p, df, peer_dfs)
                r = df[p].values - y_hat
            ...
            # Compute conditional innovations for T
            u_T = np.zeros(n_rows)
        ...
        # FINAL INTEGRATION RULE (Section 11):
        # FINAL = specialized fault OR corrected drift detector OR raw missingness
