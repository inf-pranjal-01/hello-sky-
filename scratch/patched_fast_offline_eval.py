...
def _score_and_report(featured: pd.DataFrame, label: str, n_dropped: int, silent: bool = False) -> dict:
    ...
    if label == "ALL FILES COMBINED":
        ...
        if label == "ALL FILES COMBINED":
            ...
            print(f"  Frozen Value (Episode-Level)  Uncalibrated hard gate    Fixed episode catch rate  Now tracked in episodic metrics")
...
def evaluate_all(labeled_files: list | dict, artifact: dict, silent: bool = False) -> dict:
    ...
    # Forward fill so they survive featurization's completeness check
    ...
    featured.loc[raw_nans_featured, "fault_type"] = "dropout"
