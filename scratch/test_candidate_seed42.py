...
def run():
    print("Fitting clean historical models (Normal & Uncertainty)...")
    ...
    # Compute Base Engine Predictions
    frames = []
    ...
    for sid in STATION_TO_CLUSTER:
        ...
        Z_list = []
        for p in params:
            y_hat, _ = normal_model.predict_target(sid, p, df, peer_dfs)
            r = df[p].values - y_hat
    ...
    print(f"Candidate Precision: {m_cand['precision']*100:.2f}% | Recall: {m_cand['recall']*100:.2f}% | F1: {m_cand['f1']:.4f} | F1*: {ep_cand.latency_aware_f1:.4f}")
    ...
    # Also check base alone
    m_base = _score_and_report(feat_cand.assign(__predicted=base_predicted), 'ALL FILES COMBINED', 0, silent=True)
    print(f"Base Alone Precision: {m_base['precision']*100:.2f}% | Recall: {m_base['recall']*100:.2f}% | TP: {m_base['tp']} | FP: {m_base['fp']}")
