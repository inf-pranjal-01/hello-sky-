...
class PeerContrastDriftDetector:
    """
    Evaluates peer-contrasted conditional innovation:
    u_contrast = u_i - median_{j in peers}(u_j)
    With Bonferroni-calibrated GLRT threshold and consecutive confirmation persistence.
    """
    def __init__(self, glrt_thresh=28.0, cusum_thresh=24.0, allowance=0.55, decay=0.88, persist_req=3):
    ...
    def process_station_stream(self, u_contrast, sig_arr, timestamps):
        n = len(u_contrast)
        sig = np.maximum(0.15, np.asarray(sig_arr, dtype=float))
        z_u = u_contrast / sig
        ...
        for i in range(n):
            ...
            for W in range(min_w, max_avail_w + 1, 3):
                idx_s = i - W + 1
                z_seg = z_u[idx_s:i+1]
            ...
            glrt_trigger = (max_lam >= self.glrt_thresh and abs(best_b) >= 0.10 and abs(z_u[i]) >= 2.5 and is_coherent)
            cusum_trigger = (active_cum >= self.cusum_thresh and abs(z_u[i]) >= 2.8 and is_coherent)
            ...
            if state == 0:
                ...
                if trigger_streak == 0 and abs(z_u[i]) < 1.0:
                    state = 0
...
def run_test():
    ...
    detector = PeerContrastDriftDetector(glrt_thresh=28.0, cusum_thresh=24.0, allowance=0.55, persist_req=3)
    
    # Pre-calculate u_T for all stations to compute peer contrasts
    u_T_dict = {}
    sig_T_dict = {}
    for sid in STATION_TO_CLUSTER:
        ...
        for i in range(n_rows):
            ...
            u_T[i] = u
            sig_cond_T[i] = s
        u_T_dict[sid] = np.nan_to_num(u_T, nan=0.0)
        sig_T_dict[sid] = np.nan_to_num(sig_cond_T, nan=1.0)
    ...
    for sid in STATION_TO_CLUSTER:
        ...
        # Contrast with peer median
        peer_u_stack = np.column_stack([u_T_dict[pid] for pid in peer_ids])
        peer_u_med = np.median(peer_u_stack, axis=1)
        u_contrast = u_T_dict[sid] - peer_u_med

        res_det = detector.process_station_stream(u_contrast, sig_T_dict[sid], df['timestamp'].values)
        df_stn = pd.DataFrame({
            'station_id': sid,
