...
def run_tier1_and_tier2_rules(featured, thresholds, spatial_z_dict):
    ...
    for station_id, g in df.groupby("station_id", sort=False):
        ...
        for i in range(m):
            ...
            # When both T and RH surge simultaneously far above peers:
            mv_coupled = (z_T >= 1.6 and z_RH >= 1.6 and (z_T * z_RH >= 3.0) and abs(z_P) < 2.5)
            mv_level = (
            ...
            for col, prefix in prefixes:
                ...
                if mv_confirmed and prefix in ("temp", "humidity"):
                    ...
                    evidence.append(("multivariate_inconsistency", 65.0))
                if frozen:
...
def test_multichannel_system(seed=42):
    print(f"=== TESTING SPECIALIZED MULTIVARIATE & MULTI-CHANNEL DRIFT (SEED {seed}) ===")
    ...
    theta_glrt = 28.0
    theta_cusum = 22.0
    detector_T = ConditionalMultivariateDriftDetector(glrt_thresh=theta_glrt, cusum_thresh=theta_cusum, allowance=0.45, persist_req=2)
    detector_RH = ConditionalMultivariateDriftDetector(glrt_thresh=theta_glrt, cusum_thresh=theta_cusum, allowance=0.45, persist_req=2)
    detector_P = ConditionalMultivariateDriftDetector(glrt_thresh=theta_glrt, cusum_thresh=theta_cusum, allowance=0.45, persist_req=2)
    ...
    for sid in STATION_TO_CLUSTER:
        ...
        # Innovations for all 3 channels:
        # Channel 0: Temperature, Channel 1: Humidity, Channel 2: Pressure
    ...
    featured_base, row_hard, row_rule_conf, row_fault_type = run_tier1_and_tier2_rules(featured.copy(), thresholds, spatial_z_dict)
    ...
    print("\n--- RESULTS ON SEED 42 WITH MULTI-CHANNEL DRIFT & SPECIALIZED MULTIVARIATE ---")
...
if __name__ == '__main__':
    test_multichannel_system(seed=42)
