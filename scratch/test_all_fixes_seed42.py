...
# ==============================================================================
# INTEGRATED ENGINE WITH 4 ROOT-CAUSE FIXES
# ==============================================================================
...
def run_calibrated_rules(featured, data):
    ...
    for station_id, g in df.groupby("station_id", sort=False):
        ...
        # Stagnation buffers for frozen detection (variance/range over 3-4 steps)
        ...
        for i in range(m):
            ...
            # Multivariate Fix: Level gating + persistence
            mv_level = (
                not np.isnan(temp_dev) and not np.isnan(humidity_dev)
                and abs(temp_dev) >= 0.50 and abs(humidity_dev) >= 1.20
            ...
            for col, prefix in prefixes:
                ...
                # Frozen Fix: Range stagnation over 3 consecutive steps
                val_history[prefix].append(val)
                eps_freeze = 0.08 if prefix in ("temp", "pressure") else 0.15
                if len(val_history[prefix]) >= 3 and not dropout:
                    h_arr = np.array(val_history[prefix])
                    frozen = (np.max(h_arr) - np.min(h_arr) <= eps_freeze)
                else:
                    frozen = False

                # Spike Fix: Physical step jump evaluation
...
print("Testing Seed 42 with fully synchronized architecture...", flush=True)
