...
def score_reading(raw_reading: dict, history_df: pd.DataFrame, artifact: dict,
    ...
    # Track A (blueprint §1) — fault_helper live-path wiring with causal neighbor context.
    ...
    if fault_helper_artifact is not None:
        try:
            ...
            st_id = raw_reading.get("station_id") or (history_df["station_id"].iloc[-1] if not history_df.empty and "station_id" in history_df.columns else "UNKNOWN")
            fh_res = score_live_fault_helpers(st_id, raw_reading, history_df, neighbor_buffers, fault_helper_artifact)

            # 1. Frozen channel helper alert (with validated spatial gate temp_activity_gap >= 0.6C)
            if fh_res.get("frozen_helper_alert"):
                is_anomaly = True
                if fault_type is None or fault_type == "none":
                    fault_type = "frozen_value"
                score_pct = max(score_pct, 90.0)
                severity = score_to_severity(score_pct)
                decision_basis = "statistical_rule"

            # 2. General ExtraTrees helper
            fh_prob = fh_res.get("fh_prob")
            if fh_prob is not None:
