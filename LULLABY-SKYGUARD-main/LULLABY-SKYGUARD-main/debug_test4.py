...
times = [pd.Timestamp('2024-06-01T05:00:00Z') + pd.Timedelta(hours=i) for i in range(8)]
...
swing_neighbors = {
    'AWS-CHN-101': pd.DataFrame([
        {'station_id': 'AWS-CHN-101', 'timestamp': t, 'temperature_c': 30.0, 'pressure_hpa': 1005.0, 'humidity_pct': 70.0}
        for t in times[:-1]
    ...
    'AWS-CHN-103': pd.DataFrame([
        {'station_id': 'AWS-CHN-103', 'timestamp': t, 'temperature_c': 30.0, 'pressure_hpa': 1005.0, 'humidity_pct': 70.0}
        for t in times[:-1]
