...
# Replace the 'if fault_helper_artifact is not None and not is_anomaly:' block
old_block = """    if fault_helper_artifact is not None and not is_anomaly:
        try:
...
new_block = """    if fault_helper_artifact is not None:
        try:
            from config import HELPER_ALERT_THRESHOLD
            from model.fault_helper import feature_columns, build_network_features
            single_row = pd.DataFrame([{**raw_reading, "is_anomaly": False, "fault_type": None}])
